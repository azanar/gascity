...
func TestCityRuntimeEnsureManagedDoltPublishedForTickCallsHealthWhenManagedPortMissing(t *testing.T) {
	...
	origHealth := cityRuntimeHealthBeadsProvider
	origOwned := cityRuntimeManagedDoltLifecycleOwned
	origPort := cityRuntimeCurrentManagedDoltPort
	t.Cleanup(func() {
		cityRuntimeHealthBeadsProvider = origHealth
		cityRuntimeManagedDoltLifecycleOwned = origOwned
		cityRuntimeCurrentManagedDoltPort = origPort
	})

	healthCalls := 0
	cityRuntimeHealthBeadsProvider = func(cityPath string) error {
		healthCalls++
	...
	cityRuntimeManagedDoltLifecycleOwned = func(cityPath string) (bool, error) {
		if cityPath != "/tmp/test-city" {
			t.Fatalf("owned cityPath = %q, want %q", cityPath, "/tmp/test-city")
		}
		return true, nil
	}
	cityRuntimeCurrentManagedDoltPort = func(cityPath string) string {
		if cityPath != "/tmp/test-city" {
	...
	cr := &CityRuntime{cityPath: "/tmp/test-city", stderr: io.Discard}
...
func TestCityRuntimeEnsureManagedDoltPublishedForTickSkipsHealthWhenManagedPortPresent(t *testing.T) {
	...
	healthCalls := 0
	cityRuntimeHealthBeadsProvider = func(string) error {
		healthCalls++
		return nil
	}
	cityRuntimeManagedDoltLifecycleOwned = func(string) (bool, error) {
		return true, nil
	}
	cityRuntimeCurrentManagedDoltPort = func(string) string {
		return "3307"
	}
